...
1. Poisoned document instructions are detected and blocked.
...
5. Poisoned document writes are rejected before connector execution.
...
### Protected poisoned-document flow

1. Click **Reset Demo**.
2. Click **Simulate Migration → EKS**.
3. Click **Inject poisoned document**.
4. Click **Run Scan**.

Expected results:

1. AgentFirewall changes from `ATTACK ARMED` to `THREAT CONTAINED`.
2. The risk score is critical.
3. The source is `confluence://CONF-121`.
4. The trace shows provenance, instruction hierarchy, sensitive capability, destination, and concealment checks.
5. The activity timeline reports that poisoned instructions were quarantined.
...
8. The proposed repair does not contain the malicious instruction or external URL.
...
The firewall endpoint should contain the blocked Confluence incident after the poisoned-document scan.
...
To test model-generated explanations, configure either `OPENAI_API_KEY` or `OPENROUTER_API_KEY`, restart the stack, and repeat the protected poisoned-document flow. The quarantined instruction must remain absent from model input and the proposed document repair.
